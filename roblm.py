...
if __name__ == "__main__":
    ...
    if args.train:
        ...
        dl = DataLoader(ds, shuffle=True, batch_size=2 if not args.train_rl else 1)
        ...
        if args.train_rl:
            ...
            for epoch in range(args.train_epochs):
                for batch in dl:
                    ...
                    #reward = -torch.sum(torch.tensor([Eval.aux_loss(Eval.proc_instructions(label), Eval.proc_instructions(pred))
                    #                        for label, pred in zip(preds_text, labels_text)]))
                    reward = torch.tensor([1])
                    print(reward)

                    train_stats = ppo_trainer.step(labels, outputs, reward)
                    print(train_stats)
